...
class ContactsController @Inject(
  ...
  def listAllContacts = (loggingAction andThen securedAction).async { implicit request =>
    contactsDao.listUserContacts(request.userId) map { c =>
      Ok(Json.toJson(c))
    }
  }

  def createContact = (loggingAction andThen securedAction).async(parse.json) {
    implicit request =>
      request.body
        .validate[CreateContactForm](CreateContactForm.reader) match {
        case JsSuccess(formData, _) =>
          contactsDao
            .createContact(request.userId,
                           formData.firstName,
                           formData.lastName)
            .map { c =>
  ...
  def updateContact(id: Long) =
    (loggingAction andThen securedAction).async(parse.json) {
      implicit request =>
  ...
  def deleteContact(id: Long) = (loggingAction andThen securedAction).async {
    implicit request =>
      contactsDao.findById(id) flatMap {
        case Some(contact) =>
          if (contact.owner == request.userId) {
